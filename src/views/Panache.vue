...
export default {
  ...
  data() {
    return {
      ...
      cpus: Math.min(4, navigator.hardwareConcurrency),
      workers: {},
      sliceLoaded: 0,
...
//        'grid-template-rows': `auto ${this.haTrackHeight}px ${this.gridGapSize}px ${this.autoComputeMatrixHeight}px`,
  ...
  methods: {
    setDataSlice() {
      let workers = this.$store.getters.workers;
      let blocSize = Math.round(this.chromData.length / this.$store.getters.cpus);

      for (let index in workers) {
        let offset = index * blocSize;
        let slice = this.chromData.slice(offset, offset + blocSize);

        console.log('Worker slice length', slice.length);
    ...
    filterData() {
      if (typeof this.chromData[0] != 'undefined') {
        ...
        let workers = this.$store.getters.workers;
        let cpus = Array(this.$store.getters.cpus).keys();
        ...
        for (let index of cpus) {
          ...
          worker.onmessage = result => {
            ...
            this.filteredData = [].concat(this.filteredData, result.data);

            if (done === this.$store.getters.cpus) {
